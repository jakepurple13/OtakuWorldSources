package com.programmersbox.mangaread
...
abstract class Madara(
    ...
    private fun headersBuilder(): Headers.Builder = Headers.Builder()
        .add(
            "User-Agent",
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:77.0) Gecko/20100101 Firefox/78.0$userAgentRandomizer"
        )
        .add("Referer", "$baseUrl/")
    ...
    override suspend fun itemInfo(model: ItemModel): InfoModel {
        ...
        return InfoModel(
            ...
            chapters = chapterListParse(
                client.newCall(GET(model.url, headersBuilder().build())).execute(), model.url
            ),
            description = description,
            imageUrl = doc.select("div.summary_image img").first()?.let { imageFromElement(it) }
                .orEmpty(),
            source = this
        )
    }
    ...
    override suspend fun chapterInfo(chapterModel: ChapterModel): List<Storage> {
        val doc =
            client.newCall(GET(chapterModel.url, headersBuilder().build())).execute().asJsoup()
